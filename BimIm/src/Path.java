...
public class Path {
	...
	public Path(Arcs firstArc) {
		...
		this.sommets.add(firstArc.getSommetDestination().getPixelNumber()); //On ajoute l'indice de la destination de l'arc
	...
	public boolean addPath(Arcs arc) { //Avant d'ajouter un arc, on verifie qu'on peut ajouter le sommet de destiantion
		if(this.addsommet(arc.getSommetDestination().getPixelNumber()))
			return (this.path.add(arc)); //Retroune true si on à réussie à ajouter l'arc
		else return false;
	}
	
	public void removePath(Arcs arc) {
		this.removeSommet(arc.getSommetDestination().getPixelNumber());
